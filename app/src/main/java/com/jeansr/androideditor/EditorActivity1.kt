...
class EditorActivity : AppCompatActivity() {
    ...
    private fun compilePreviewWithResources() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val extraIncludes = preparedLibraries.mapNotNull { it.staticResApk }.joinToString(" ") { "-I ${it.absolutePath}" }
            val linkCmd = "${aapt2File.absolutePath} link -I ${androidJarFile.absolutePath} -I ${materialLibFile.absolutePath} $extraIncludes -o ${outputApk.absolutePath} --manifest ${manifestFile.absolutePath} ${compiledZip.absolutePath} --auto-add-overlay --allow-reserved-package-id --no-static-lib-packages"
    ...
    private fun parseProjectDependencies(): List<MavenResolver.Coordinate> {
        val gradleFile = listOf(File(projectRoot, "app/build.gradle.kts"), File(projectRoot, "app/build.gradle"))
            .firstOrNull { it.exists() } ?: return emptyList()

        val pattern = Regex("""(?:implementation|api)\s*[\("']([\w.\-]+):([\w.\-]+):([\w.\-]+)[\)"']""")
        return pattern.findAll(gradleFile.readText())
            .map { MavenResolver.Coordinate(it.groupValues[1], it.groupValues[2], it.groupValues[3]) }
            .filterNot { it.groupId.startsWith("com.google.android.material") }
            .distinct()
            .toList()
    }
...
*/
    private suspend fun setupCompiler() {
        ...
        libraryPreparer = LibraryPreparer(
            aapt2File = aapt2File,
